...
Four layers of agentic time series tooling. Each layer is a FastMCP server
...
Per-layer installs: `.[analyst]`, `.[forecaster]`, `.[deploy]`, `.[monitor]` if you don't want every dependency.
...
- **Tests use `pytest.importorskip("statsmodels")` / `"sklearn"`** in the layers that need them (`analyst`, `forecaster`, `deploy`), so `pip install -e .` (core deps only) doesn't break the whole suite. `monitor` and `data_prep` tests have no such guard — keep it that way, they're meant to run everywhere.
...
- `src/agentic_ts_toolkit/skills/` — the four `SKILL.md` files, bundled as package data via `skills_dir()` in `__init__.py`.
- `openclaw.config.snippet.jsonc` — reference config for wiring all four servers + a model into OpenClaw.
